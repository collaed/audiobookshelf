<template>
  <div>
    <div v-if="narrators?.length" class="flex py-0.5 mt-4">
      ...
      <div class="max-w-[calc(100vw-10rem)] overflow-hidden text-ellipsis">
        <template v-for="(narrator, index) in narrators">
          <NuxtLink :key="narrator" :to="`/library/${libraryId}/bookshelf?filter=narrators.${$encode(narrator)}`" class="hover:underline">{{ narrator }}</nuxt-link
    ...
    <div v-if="publisher" role="paragraph" class="flex py-0.5">
      ...
      <div>
        <NuxtLink :to="`/library/${libraryId}/bookshelf?filter=publishers.${$encode(publisher)}`" class="hover:underline">{{ publisher }}</NuxtLink>
      </div>
    </div>
    ...
    <div class="flex py-0.5" v-if="genres.length">
      ...
      <div class="max-w-[calc(100vw-10rem)] overflow-hidden text-ellipsis">
        <template v-for="(genre, index) in genres">
          <NuxtLink :key="genre" :to="`/library/${libraryId}/bookshelf?filter=genres.${$encode(genre)}`" class="hover:underline">{{ genre }}</nuxt-link
    ...
    <div class="flex py-0.5" v-if="tags.length">
      ...
      <div class="max-w-[calc(100vw-10rem)] overflow-hidden text-ellipsis">
        <template v-for="(tag, index) in tags">
          <NuxtLink :key="tag" :to="`/library/${libraryId}/bookshelf?filter=tags.${$encode(tag)}`" class="hover:underline">{{ tag }}</nuxt-link
    ...
    <div v-if="language" class="flex py-0.5">
      ...
      <div>
        <NuxtLink :to="`/library/${libraryId}/bookshelf?filter=languages.${$encode(language)}`" class="hover:underline">{{ language }}</NuxtLink>
      </div>
    </div>
