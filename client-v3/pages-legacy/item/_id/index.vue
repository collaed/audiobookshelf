          ...
          <div class="flex justify-center">
            <div class="mb-4">
              ...
              <template v-for="(_series, index) in seriesList">
                <NuxtLink :key="_series.id" :to="`/library/${libraryId}/series/${_series.id}`" class="hover:underline font-sans text-gray-300 text-lg leading-7">{{ _series.text }}</nuxt-link
              ...
              <p v-else-if="authors.length" class="mb-2 mt-0.5 text-gray-200 text-lg md:text-xl max-w-[calc(100vw-2rem)] overflow-hidden text-ellipsis">
                {{ $getString('LabelByAuthor', ['']) }}<NuxtLink v-for="(author, index) in authors" :key="index" :to="`/author/${author.id}`" class="hover:underline">{{ author.name }}<span v-if="index < authors.length - 1">,&nbsp;</span></NuxtLink>
              </p>
