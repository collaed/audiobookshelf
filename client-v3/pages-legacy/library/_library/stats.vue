            ...
            <template v-for="genre in top5Genres">
              <div :key="genre.genre" class="w-full py-2">
                <div class="flex items-end mb-1">
                  ...
                  <div class="grow" />
                  <NuxtLink :to="`/library/${currentLibraryId}/bookshelf?filter=genres.${$encode(genre.genre)}`" class="text-base text-white/70 hover:underline">
                    {{ genre.genre }}
                  </NuxtLink>
                </div>
              ...
              <div :key="author.id" class="w-full py-2">
                <div class="flex items-center mb-1">
                  <p class="text-sm text-white/70 w-36 pr-2 truncate">
                    {{ index + 1 }}.&nbsp;&nbsp;&nbsp;&nbsp;<NuxtLink :to="`/author/${author.id}`" class="hover:underline">{{ author.name }}</NuxtLink>
                  </p>
              ...
              <div :key="index" class="w-full py-2">
                <div class="flex items-center mb-1">
                  <p class="text-sm text-white/70 w-44 pr-2 truncate">
                    {{ index + 1 }}.&nbsp;&nbsp;&nbsp;&nbsp;<NuxtLink :to="`/item/${ab.id}`" class="hover:underline">{{ ab.title }}</NuxtLink>
                  </p>
                  <div class="grow rounded-full h-2.5 bg-primary/0 overflow-hidden">
